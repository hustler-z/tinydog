...
{
	writel(value, (xmem_t)addr);
}

static inline int serial_in_shift(void *addr, int shift)
{
	return readl((xmem_t)addr);
}
...
static void __serial_setbrg(struct uart *com_port, int baud_divisor)
{
     ...
     */
	int lcr_val = serial_in((xmem_t)&com_port->lcr) & ~UART_LCR_BKSE;

	serial_out(UART_LCR_BKSE | lcr_val, (xmem_t)&com_port->lcr);
	serial_out(baud_divisor & 0xFF, (xmem_t)&com_port->dll);
	serial_out((baud_divisor >> 8) & 0xFF, (xmem_t)&com_port->dlm);
	serial_out(lcr_val, (xmem_t)&com_port->lcr);
}
...
{
	while (!(serial_in((xmem_t)&com_port->lsr) & UART_LSR_TEMT))
		;

	serial_out(0x00, (xmem_t)&com_port->ier);
	serial_out(UART_MCRVAL, (xmem_t)&com_port->mcr);
	serial_out(__serial_getfcr(com_port), (xmem_t)&com_port->fcr);
	/* initialize serial config to 8N1 before writing baudrate
     */
	serial_out(UART_LCRVAL, (xmem_t)&com_port->lcr);
	if (baud_divisor != -1)
...
{
	while (!(serial_in((xmem_t)&com_port->lsr) & UART_LSR_THRE))
		;
	serial_out(c, (xmem_t)&com_port->thr);
}

char __serial_getc(struct uart *com_port)
{
	while (!(serial_in((xmem_t)&com_port->lsr) & UART_LSR_DR))
        ;

	return serial_in((xmem_t)&com_port->rbr);
}

int __serial_tstc(struct uart *com_port)
{
	return (serial_in((xmem_t)&com_port->lsr) & UART_LSR_DR) != 0;
}
