...
extern void __asm_outc__(const char c);
...
int __init prev_print_init(void) {
    __outs__ = prev_print;
...
static void post_print(const char *msg) {
    size_t len = strlen(msg);

    while (len-- > 0) {
        if (*msg == '\n')
            __debug_uart_putc((int)'\r');

        __debug_uart_putc((int)*msg);
...
// @Hustler
int __init post_print_init(void) {
