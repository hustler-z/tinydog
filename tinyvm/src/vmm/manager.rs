...
use crate::arch::{
	is_boot_core, power_arch_vm_shutdown_secondary_cores, IntCtrl,
	InterruptController,
};
...
use crate::kernel::{
	active_vcpu_id, active_vm, current_cpu, push_vm, vm, vm_if_get_state,
	vm_if_set_ivc_arg, vm_if_set_ivc_arg_ptr, vm_if_set_type, vm_ipa2pa,
	Scheduler, Vm, VM_NUM_MAX,
};
use crate::kernel::{active_vm_id, vm_if_get_cpu_id};
use crate::kernel::{hvc_send_msg_to_vm, HvcGuestMsg, HvcManageMsg};
use crate::kernel::{
	ipi_send_msg, IpiInnerMsg, IpiMessage, IpiType, IpiVmmMsg,
};
use crate::utils::{bit_extract, memcpy, memset};
use crate::vmm::{
	vmm_assign_vcpu_percore, vmm_boot, vmm_cpu_remove_vcpu, vmm_init_image,
	vmm_setup_config,
};
...
pub fn vmm_push_vm(vm_id: usize) -> Result<Arc<Vm>, ()> {
	info!("push vm[{}] on hcpu[{}]", vm_id, current_cpu().id);
...
pub fn vmm_init_gvm(vm_id: usize) {
	...
	if is_boot_core(current_cpu().id)
		|| (active_vm_id() == 0 && active_vm_id() != vm_id)
	{
...
pub fn vmm_boot_vm(vm_id: usize) {
	if let Some(phys_id) = vm_if_get_cpu_id(vm_id) {
		if phys_id != current_cpu().id {
			...
			let state =
				CPU_IF_LIST.lock().get(phys_id).unwrap().state_for_start;
			if state == CpuState::CpuInv {
				let vmpidr =
					vm(vm_id).unwrap().pcpuid_to_vcpuid(phys_id).unwrap();
			...
			match current_cpu().vcpu_array.pop_vcpu_through_vmid(vm_id) {
				None => {
					error!(
						"failed to get vcpu for vm[{}] not configured",
						vm_id
					);
				}
				Some(vcpu) => {
		...
		error!("failed to get cpu id for vm[{}] not configured", vm_id);
	}
}
...
pub fn vmm_reboot() {
	...
	for idx in 0..vm.mem_region_num() {
		info!(
            "hcpu[{}] (vm [{}] vcpu {}) reset mem region start {:08x} size {:08x}",
            current_cpu().id,
            vm.id(),
            active_vcpu_id(),
            vm.pa_start(idx),
...
pub fn get_vm_id(id_ipa: usize) -> bool {
	...
	if id_pa == 0 {
		error!("illegal id_ipa {:08x}", id_ipa);
		return false;
	}
...
pub fn vmm_list_vm(vm_info_ipa: usize) -> Result<usize, ()> {
	...
	if vm_info_pa == 0 {
		error!("illegal vm_info_ipa {:08x}", vm_info_ipa);
		return Err(());
	}
...
pub fn vmm_ipi_handler(msg: IpiMessage) {
	info!("hcpu[{}] received IPI", current_cpu().id);
	match msg.ipi_message {
		...
		IpiInnerMsg::VmmPercoreMsg(msg) => match msg.event {
			VmmPercoreEvent::VmmAssignCpu => {
				debug!(
					"hcpu[{}] receive assigned vcpu request for vm[{}]",
					current_cpu().id,
					msg.vm.id()
				);
