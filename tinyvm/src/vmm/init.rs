...
use crate::kernel::{
    add_async_used_info, cpu_idle, current_cpu, iommu_add_device, IpiInnerMsg, IpiType, VmPa,
    VmType,
};
...
pub fn vmm_init_image(vm: &Vm) -> bool {
    ...
    if config.ramdisk_load_ipa() != 0 {
        ...
        let dst =
            unsafe { core::slice::from_raw_parts_mut((vm.pa_start(0) + offset) as *mut u8, len) };
