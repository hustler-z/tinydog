...
/**
 * This module is used to forward the unilib request from Unitinyvm to MVM.
 * Note:
 ...
 *  But for different Unitinyvms runs on one single CPU, stucking in EL2 may cause a big efficiency problem,
...
use crate::kernel::{
	active_vm, vm_ipa2pa, HVC_UNILIB_FS_INIT, HVC_UNILIB_FS_LSEEK,
	HVC_UNILIB_FS_UNLINK,
};
use crate::kernel::{hvc_send_msg_to_vm, HvcGuestMsg, HvcUniLibMsg};
use crate::kernel::{
	HVC_UNILIB_FS_CLOSE, HVC_UNILIB_FS_OPEN, HVC_UNILIB_FS_READ,
	HVC_UNILIB_FS_WRITE,
};
use crate::utils::{memcpy_safe, sleep};

pub static UNILIB_FS_LIST: Mutex<BTreeMap<usize, UnilibFS>> =
...
pub struct UnilibFSCfg {
	...
	name: [u8; 32],
	/// The "client" VM who owns this `UnilibFSCfg`, currently it should be our Unitinyvm.
	vmid: usize,
...
/// The MVM's setting up process mainly happens in tinyvm-cli.
