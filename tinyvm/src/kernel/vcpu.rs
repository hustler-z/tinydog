...
#[cfg(target_arch = "riscv64")]
use crate::arch::{
	get_trapframe_for_hart, SSTATUS_FS, SSTATUS_SPIE, SSTATUS_SPP, SSTATUS_VS,
	TP_NUM,
};
use crate::arch::{
	ArchTrait, ContextFrame, ContextFrameTrait, GicContext, VmContext,
};
...
static IDLE_THREAD: spin::Lazy<[Option<IdleThread>; PLATFORM_CPU_NUM_MAX]> =
	spin::Lazy::new(|| {
		...
		for i in 0..PLATFORM_CPU_NUM_MAX {
			...
			{
				ctx.sstatus =
					SSTATUS_SPIE | SSTATUS_SPP | SSTATUS_FS | SSTATUS_VS;
...
pub fn run_idle_thread() {
	...
	unsafe {
		crate::utils::memcpy(
			...
			&(IDLE_THREAD[current_cpu().id].unwrap().ctx) as *const _
				as *const u8,
...
pub fn vcpu_run(announce: bool) -> ! {
	...
	#[cfg(target_arch = "riscv64")]
	{
		...
		current_cpu().ctx_mut().unwrap().sscratch =
			get_trapframe_for_hart(current_cpu().id);
