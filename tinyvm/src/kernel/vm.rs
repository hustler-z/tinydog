...
impl VmInnerConst {
	fn new(id: usize, config: VmConfigEntry, vm: Weak<Vm>) -> VmInnerConst {
		...
		info!("vm[{}] hcpu list {:?}", id, phys_id_list);
	...
	fn init_devices(&mut self, vm: Weak<Vm>) -> bool {
		...
		for (idx, emu_cfg) in
			self.config.emulated_device_list().iter().enumerate()
		{
			let dev = match emu_cfg.emu_type {
				...
				EmuDeviceTGicd => {
					...
					emu_intc_init(emu_cfg, &self.vcpu_list).map(|vgic| {
						self.arch_intc_dev =
							vgic.clone().into_any_arc().downcast::<Vgic>().ok();
						vgic
					})
				}
				#[cfg(all(feature = "plic", target_arch = "riscv64"))]
				...
				| EmuDeviceTVirtioNet
				| EmuDeviceTVirtioBlk => emu_virtio_mmio_init(vm.clone(), emu_cfg),
				EmuDeviceTIOMMU => emu_iommu_init(emu_cfg),
				EmuDeviceTTinyvm => {
					tinyvm_init(vm.clone(), emu_cfg.base_ipa, emu_cfg.length)
				}
				_ => {
					panic!(
						"unknown emulate device type {:?}",
						emu_cfg.emu_type
					);
				}
			};
			...
			if let Ok(emu_dev) = dev {
				if self.emu_devs.iter().any(|dev| {
					emu_dev.address_range().contains(&dev.address_range().start)
						|| dev
							.address_range()
							.contains(&emu_dev.address_range().start)
				}) {
					panic!(
                        "duplicated emulate address region: previous address {:x?}",
				...
				panic!("failed to initialize emulate device");
			}
			...
			info!(
				"vm[{}] emulate device: [id={}, ipa={:08x}]",
				self.id, idx, emu_cfg.base_ipa
			);
		}
		// Passthrough irqs
...
impl Vm {
	...
	pub fn iommu_ctx_id(&self) -> usize {
		...
		match vm_inner.iommu_ctx_id {
			None => {
				panic!("vm[{}] ain't have iommu context bank", self.id());
			}
			Some(id) => id,
		}
	}
	...
	pub fn med_blk_id(&self) -> usize {
		match self.config().mediated_block_index() {
			None => {
				panic!("vm[{}] ain't have mediated block", self.id());
			}
			Some(idx) => idx,
		}
	}
	...
	pub fn vcpu(&self, index: usize) -> Option<&Vcpu> {
		match self.vcpu_list().get(index) {
			...
			None => {
				error!(
					"vcpu[{}] larger than vcpu list length {}",
					index,
					self.vcpu_list().len()
				);
				None
	...
	pub fn pt_map_range(
		...
		match &vm_inner.pt {
			...
			None => {
				panic!("vm[{}] page table empty", self.id());
			}
		}
	}
	...
	pub fn pt_set_access_permission(
		&self,
		ipa: usize,
		ap: usize,
	) -> (usize, usize) {
	...
	pub fn vgic(&self) -> &Vgic {
		...
		panic!("vm{} can not find vgic", self.id());
	}
	...
	pub fn vaplic(&self) -> &VAPlic {
		...
		panic!("vm{} can not find vaplic", self.id());
	}
	...
	pub fn ipa2pa(&self, ipa: usize) -> usize {
		if ipa == 0 {
			error!("vm[{}] access invalid ipa {:08x}", self.id(), ipa);
			return 0;
		}
...
pub fn vm_ipa2pa(vm: &Vm, ipa: usize) -> usize {
	if ipa == 0 {
		error!("vm[{}] access invalid ipa {:08x}", vm.id(), ipa);
		return 0;
	}
