...
pub static GLOBAL_PLIC: Mutex<PLIC> =
	Mutex::new(PLIC::new(PLIC_BASE_ADDR as usize));
#[cfg(all(feature = "aia", target_arch = "riscv64"))]
pub static GLOBAL_APLIC: Mutex<APLIC> =
	Mutex::new(APLIC::new(APLIC_BASE_ADDR as usize, APLIC_SIZE as usize));
...
impl InterruptController for IntCtrl {
	...
	fn init() {
		...
		#[cfg(feature = "plic")]
		{
			...
			locked.set_threshold(
				crate::arch::PLICMode::Machine,
				current_cpu().id,
				0,
			);
	...
	fn clear() {
		...
		loop {
			#[cfg(feature = "plic")]
			{
				let irq = GLOBAL_PLIC
					.lock()
					.get_claim(super::PLICMode::Machine, current_cpu().id);
				if irq == 0 {
					// TODO: not clearing sip，maybe no need?
					break;
				} else {
					GLOBAL_PLIC.lock().set_complete(
						super::PLICMode::Machine,
						current_cpu().id,
						irq,
					);
				}
...
pub fn psci_ipi_handler(msg: IpiMessage) {
	...
	match msg.ipi_message {
		IpiInnerMsg::Power(power_msg) => {
			...
			let target_vcpu = match current_cpu()
				.vcpu_array
			...
			match power_msg.event {
				...
				PowerEvent::PsciIpiCpuOn => {
					if target_vcpu.state() as usize
						!= VcpuState::Invalid as usize
					{
						warn!(
