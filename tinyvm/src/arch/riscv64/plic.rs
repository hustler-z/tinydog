	...
	fn get_enable_array_addr(
		...
		let res = match hart {
			...
			_ => match mode {
				PLICMode::Machine => {
					self.base_addr
						+ PLIC_ENABLE_BEGIN + 0x80
						+ 0x100 * (hart - 1) + 0x4 * (irq / 32)
				}
				PLICMode::Supervisor => {
					self.base_addr
						+ PLIC_ENABLE_BEGIN + 0x100
						+ 0x100 * (hart - 1) + 0x4 * (irq / 32)
				}
			},
		};
	...
	fn get_prio_claim_baseaddr(
		...
		let res = match hart {
			...
			_ => match mode {
				PLICMode::Machine => {
					self.base_addr
						+ PLIC_THRESHOLD_CLAIM_BEGIN
						+ 0x1000 + 0x2000 * (hart - 1)
				}
				PLICMode::Supervisor => {
					self.base_addr
						+ PLIC_THRESHOLD_CLAIM_BEGIN
						+ 0x2000 + 0x2000 * (hart - 1)
				}
			},
		};
		#[cfg(not(feature = "board_u740"))]
...
impl PLICTrait for PLIC {
	...
	fn get_pending(&self, irq: usize) -> bool {
		...
		unsafe {
			(read_volatile((addr) as *const u32) & (1 << (irq % 32))) != 0
		}
	}
