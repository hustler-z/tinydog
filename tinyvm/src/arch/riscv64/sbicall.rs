...
use rustsbi::{
	Console, EnvInfo, HartMask, Hsm, Ipi, Pmu, Reset, RustSBI, Timer,
};
...
use crate::kernel::{
	active_vm, current_cpu, ipi_send_msg, IpiInnerMsg, IpiIntInjectMsg,
	IpiPowerMessage, IpiType, PowerEvent, VcpuState, Vm,
};
...
pub struct VmHart {
	pub env: Mutex<TinyvmSBI>,
}

#[derive(RustSBI, Default)]
pub struct TinyvmSBI {
	console: VConsole,
	timer: VTimer,
	ipi: VIpi,
	hsm: VHsm,
...
impl VmHart {
	...
	pub fn handle_ecall(
		...
		use sbi_spec::legacy::{
			LEGACY_CONSOLE_GETCHAR, LEGACY_CONSOLE_PUTCHAR,
		};
		match extension {
