...
use super::aplic::{
	APLIC_CLR_ENABLE_BASE, APLIC_CLR_ENABLE_NUM_BASE, APLIC_CLR_ENABLE_NUM_TOP,
	APLIC_CLR_ENABLE_TOP, APLIC_CLR_PENDING_BASE, APLIC_CLR_PENDING_NUM_BASE,
	APLIC_CLR_PENDING_NUM_TOP, APLIC_CLR_PENDING_TOP, APLIC_DOMAINCFG_BASE,
	APLIC_DOMAINCFG_TOP, APLIC_GENMSI_BASE, APLIC_GENMSI_TOP,
	APLIC_SET_ENABLE_BASE, APLIC_SET_ENABLE_NUM_BASE, APLIC_SET_ENABLE_NUM_TOP,
	APLIC_SET_ENABLE_TOP, APLIC_SET_IPNUM_BE_BASE, APLIC_SET_IPNUM_BE_TOP,
	APLIC_SET_IPNUM_LE_BASE, APLIC_SET_IPNUM_LE_TOP, APLIC_SET_PENDING_BASE,
	APLIC_SET_PENDING_NUM_BASE, APLIC_SET_PENDING_NUM_TOP,
	APLIC_SET_PENDING_TOP, APLIC_SOURCECFG_BASE, APLIC_SOURCECFG_TOP,
	APLIC_S_MSIADDR_BASE, APLIC_S_MSIADDR_TOP, APLIC_TARGET_BASE,
	APLIC_TARGET_TOP,
};
...
impl VAPlic {
	pub fn vm_read_register(&self, addr: usize) -> u32 {
		...
		} else if (APLIC_CLR_PENDING_BASE..=APLIC_CLR_PENDING_TOP)
			.contains(&offset)
		{
			// inclrip
		...
		/* } else if (APLIC_CLR_PENDING_NUM_BASE..=APLIC_CLR_PENDING_NUM_TOP).contains(&offset) {
			...
			panic!(
				"invalid plic register access: 0x{:x} offset: 0x{:x}",
				addr, offset
			);
		}
	}
	...
	pub fn vm_write_register(&self, addr: usize, value: u32) {
		...
		} else if (APLIC_SET_PENDING_BASE..=APLIC_SET_PENDING_TOP)
			.contains(&offset)
		{
			// setip
			panic!("setip Unexpected addr {:#x}", addr);
		} else if (APLIC_SET_PENDING_NUM_BASE..=APLIC_SET_PENDING_NUM_TOP)
			.contains(&offset)
		{
			// setipnum
			panic!("setipnum Unexpected addr {:#x}", addr);
		...
		} else if (APLIC_CLR_PENDING_NUM_BASE..=APLIC_CLR_PENDING_NUM_TOP)
			.contains(&offset)
		{
			// clripnum
			panic!("clripnum Unexpected addr {:#x}", addr);
		} else if (APLIC_SET_ENABLE_BASE..=APLIC_SET_ENABLE_TOP)
			.contains(&offset)
		{
			// setie
			panic!("setie Unexpected addr {:#x}", addr);
		} else if (APLIC_SET_ENABLE_NUM_BASE..=APLIC_SET_ENABLE_NUM_TOP)
			.contains(&offset)
		{
			// setienum
		...
		} else if (APLIC_CLR_ENABLE_BASE..=APLIC_CLR_ENABLE_TOP)
			.contains(&offset)
		{
			// clrie
		...
		} else if (APLIC_CLR_ENABLE_NUM_BASE..=APLIC_CLR_ENABLE_NUM_TOP)
			.contains(&offset)
		{
			// clrienum
		...
		} else if (APLIC_SET_IPNUM_LE_BASE..=APLIC_SET_IPNUM_LE_TOP)
			.contains(&offset)
		{
			// setipnum_le
			self.setipnum_le(value);
			// debug!("APLIC setipnum le write addr@{:#x} value@{:#x}",addr, value);
		} else if (APLIC_SET_IPNUM_BE_BASE..=APLIC_SET_IPNUM_BE_TOP)
			.contains(&offset)
		{
			// setipnum_be
...
impl EmuDev for VAPlic {
	...
	fn handler(&self, emu_ctx: &EmuContext) -> bool {
		...
		if emu_ctx.write {
			vaplic.vm_write_register(
				emu_ctx.address,
				current_cpu().get_gpr(reg_idx) as u32,
			);
		} else {
