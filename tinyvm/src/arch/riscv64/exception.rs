...
use crate::arch::{
	ldst_guest_page_fault_handler, VmHart, A0_NUM, A1_NUM, A2_NUM, A3_NUM,
	A4_NUM, A5_NUM, A6_NUM, A7_NUM,
};
...
fn ecall_handler(ctx: &mut ContextFrame) {
	...
	if (0x0A000000..=0x0AFFFFFF).contains(&eid) {
		...
		match hvc_guest_handler(
			...
			Err(_) => {
				warn!(
					"Failed to handle hvc request type 0x{:x} event 0x{:x}",
					hvc_type, event
				);
	...
	ret = SBI_VM_HART.call_once(|| VmHart::new()).handle_ecall(
		eid as usize,
		fid as usize,
		[x0, x1, x2, x3, x4, x5],
	);
