...
use super::plic::{
	MAX_HARTS, PLIC_ENABLE_BEGIN, PLIC_ENABLE_END, PLIC_MAX_IRQ,
	PLIC_PENDING_BEGIN, PLIC_PENDING_END, PLIC_PRIO_BEGIN, PLIC_PRIO_END,
	PLIC_THRESHOLD_CLAIM_BEGIN, PLIC_THRESHOLD_CLAIM_END,
};
...
impl VPlic {
	...
	fn get_pending_irq(&self, mode: PLICMode, hart: usize) -> usize {
		...
		for i in 1..=PLIC_MAX_IRQ {
			if self.get_pending(i)
				&& self.get_enable(i, mode, hart)
				&& self.get_priority(i) >= thres
			{
				return i;
			}
		}
...
impl PLICTrait for VPlic {
	...
	fn get_enable(&self, irq: usize, mode: PLICMode, hart: usize) -> bool {
		if !(irq <= PLIC_MAX_IRQ && irq > 0)
			|| hart >= MAX_HARTS
			|| (mode == PLICMode::Supervisor && hart == 0)
		{
			return false;
		}
...
impl VPlic {
	...
	pub fn vm_read_register(&self, addr: usize) -> u32 {
		...
		} else if (PLIC_THRESHOLD_CLAIM_BEGIN..=PLIC_THRESHOLD_CLAIM_END)
			.contains(&offset)
		{
			...
			#[cfg(feature = "board_u740")]
			{
				mode = if ((offset & 0x2000) != 0)
					&& ((offset & !0xfff) != PLIC_THRESHOLD_CLAIM_BEGIN)
				{
					PLICMode::Supervisor
				} else {
					PLICMode::Machine
				...
				hart =
					(offset - (PLIC_THRESHOLD_CLAIM_BEGIN - 0x1000)) / 0x2000;
			}
			#[cfg(not(feature = "board_u740"))]
			{
				mode = if (offset & 0x2000) != 0 {
