...
use crate::arch::{
	isb, sysreg_enc_addr, IntCtrl, ICC_BPR1_EL1, ICC_CTLR_EL1, ICC_DIR_EL1,
	ICC_EOIR1_EL1, ICC_IGRPEN1_EL1, ICC_PMR_EL1, ICC_SGI1R_EL1, ICC_SRE_EL1,
	ICC_SRE_EL2, ICH_AP0R0_EL2, ICH_AP0R1_EL2, ICH_AP0R2_EL2, ICH_AP1R0_EL2,
	ICH_AP1R1_EL2, ICH_AP1R2_EL2, ICH_EISR_EL2, ICH_ELRSR_EL2, ICH_HCR_EL2,
	ICH_LR0_EL2, ICH_LR10_EL2, ICH_LR11_EL2, ICH_LR12_EL2, ICH_LR13_EL2,
	ICH_LR14_EL2, ICH_LR15_EL2, ICH_LR1_EL2, ICH_LR2_EL2, ICH_LR3_EL2,
	ICH_LR4_EL2, ICH_LR5_EL2, ICH_LR6_EL2, ICH_LR7_EL2, ICH_LR8_EL2,
	ICH_LR9_EL2, ICH_MISR_EL2, ICH_VMCR_EL2, ICH_VTR_EL2,
};
...
pub const GICD_TYPER_IDBITS_MSK: usize =
	(((1 << ((GICD_TYPER_IDBITS_LEN) - 1)) << 1) - 1)
		<< (GICD_TYPER_IDBITS_OFF);
const GICD_IROUTER_AFF_MSK: usize =
	GICD_IROUTER_RES0_MSK & !GICD_IROUTER_IRM_BIT;
...
pub const GICH_LR_PID_MSK: usize =
	(((1 << ((GICH_LR_PID_LEN) - 1)) << 1) - 1) << (GICH_LR_PID_OFF);
...
pub const GICH_LR_STATE_ACT: usize =
	(2 << GICH_LR_STATE_OFF) & GICH_LR_STATE_MSK;
pub const GICH_LR_STATE_PND: usize =
	(1 << GICH_LR_STATE_OFF) & GICH_LR_STATE_MSK;
...
pub static INTERRUPT_EN_SET: Mutex<BTreeSet<usize>> =
...
impl GicDistributor {
	...
	fn global_init(&self) {
		...
		self.CTLR.set(
			prev | GICD_CTLR_ARE_NS_BIT as u32 | GICD_CTLR_ENNS_BIT as u32,
		);
	}
	...
	pub fn set_prio(&self, int_id: usize, prio: u8) {
		...
		let value =
			(prev & !(mask as u32)) | (((prio as u32) << off) & mask as u32);
	...
	pub fn set_icfgr(&self, int_id: usize, cfg: u8) {
		...
		self.ICFGR[reg_ind]
			.set((icfgr & !mask) | (((cfg as u32) << off as u32) & mask));
...
impl GicRedistributor {
	fn init(&self) {
		...
		while (self[current_cpu().id].WAKER.get()
			& GICR_WAKER_CASLEEP_BIT as u32)
			!= 0
		{}
	...
	pub fn get_prio(&self, int_id: usize, gicr_id: u32) -> usize {
		...
		let prio = (self[gicr_id as usize].IPRIORITYR[reg_id].get() as usize)
			>> off & mask;
...
impl crate::arch::InterruptContextTrait for GicState {
	fn save_state(&mut self) {
		...
		unsafe {
			ICC_SRE_EL2::write(ICC_SRE_EL2::read() & !GICC_SRE_EL2_ENABLE)
		}
	}
