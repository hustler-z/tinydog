...
pub const PTE_S2_FIELD_MEM_ATTR_NORMAL_OUTER_WRITE_BACK_CACHEABLE: usize =
	0b11 << 4;
pub const PTE_S2_FIELD_MEM_ATTR_NORMAL_OUTER_WRITE_BACK_NOCACHEABLE: usize =
	0b1 << 4;

pub const PTE_S2_FIELD_MEM_ATTR_NORMAL_INNER_WRITE_BACK_CACHEABLE: usize =
	0b11 << 2;
...
pub const PTE_S2_NORMAL: usize =
	PTE_S2_FIELD_MEM_ATTR_NORMAL_INNER_WRITE_BACK_CACHEABLE
		| PTE_S2_FIELD_MEM_ATTR_NORMAL_OUTER_WRITE_BACK_CACHEABLE
		| PTE_S2_FIELD_AP_RW
		| PTE_S2_FIELD_SH_OUTER_SHAREABLE
		| PTE_S2_FIELD_AF;

pub const PTE_S2_NORMALNOCACHE: usize =
	PTE_S2_FIELD_MEM_ATTR_NORMAL_INNER_WRITE_BACK_CACHEABLE
		...
		while ipa < (start_ipa + len) {
			...
			if !l2e.valid() {
				...
				unsafe {
					l1e.set_entry(
						pt_lvl2_idx(ipa),
						Aarch64PageTableEntry::from_pa(pte),
					);
				}
				ipa += 512 * 4096; // 2MB: 9 + 12 bits
				pa = l2e.to_pa();
			...
			if l2e.valid() {
				...
				if l3e.valid() {
					...
					unsafe {
						l2e.set_entry(
							pt_lvl3_idx(ipa),
							Aarch64PageTableEntry::from_pa(0),
						);
					}
					// check l2e
