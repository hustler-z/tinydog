...
use crate::arch::{
	at, data_abort_handler, hvc_handler, isb, smc_handler, sysreg_handler,
	ContextFrameTrait, HPFAR_EL2,
};
...
/// @Hustler
///
/// ESR_ELn   - Holds syndrome information for an exception taken
///             to ELn.
///
/// FAR_ELn   - Holds the faulting virtual address for all
///             synchronous instruction abort exceptions, data
///             abort exceptions, PC alignment fault exceptions
///             and watchpoint exceptions that taken to ELn.
///             [63:0]
///
/// HPFAR_EL2 - Holds the faulting IPA for some aborts on a stage
///             2 translation taken to EL2 and GPC exceptions
///             due to a fault on an access for a stage 2
///             translation.
///             [47:4]
/// #############################################################
#[inline(always)]
...
///              Returns the output address (OA) from an address
///              translation instruction that executed
///              successfully, or fault information if the
///              instruction did not execute successfully.
...
pub fn exception_fault_addr() -> usize {
	...
	let hpfar = if (exception_esr() & ESR_ELx_S1PTW) == 0
