...
use crate::utils::{bit_extract, device_ref::DeviceRef, FlexBitmap};
...
impl SmmuV2 {
	fn new() -> Self {
		...
		let glb_rs1 =
			unsafe { &*((smmu_base_addr + page_size) as *const SmmuGlbRS1) };
		let mut r = Self {
			glb_rs0,
			glb_rs1,
			context_s2_idx: 0,
	...
	fn init(&mut self) {
		...
		let num_pages = 1
			<< (1 + bit_extract(
				idr1,
				SMMUV2_IDR1_NUMPAGEDXB_OFF,
				SMMUV2_IDR1_NUMPAGEDXB_LEN,
			));
		let context_bank_num =
			bit_extract(idr1, SMMUV2_IDR1_NUMCB_OFF, SMMUV2_IDR1_NUMCB_LEN);
		...
		for i in 0..context_bank_num {
			...
			self.context_bank.push(unsafe {
				DeviceRef::new((context_base + page_size * i) as *const _)
			});
		}
		let stage2_context_bank_num =
			bit_extract(idr1, SMMUV2_IDR1_NUMS2CB_OFF, SMMUV2_IDR1_NUMS2CB_LEN);
	...
	fn smr_get_context(&self, smr: usize) -> usize {
		bit_extract(
			self.glb_rs0.S2CR[smr].get() as usize,
			S2CR_CBNDX_OFF,
			S2CR_CBNDX_LEN,
		)
	}
	...
	fn compatible_smr_exists(
		...
		for smr in 0..self.smr_num {
			...
			if bit == 0 {
				...
				if diff_id != 0 {
					if group
						|| (self.smr_is_group(smr)
							&& (mask_r == mask || mask_r == smr_mask))
						|| (context_id == self.smr_get_context(smr))
					{
						if mask > smr_mask {
							self.smr_alloc_bitmap
								.as_mut()
								.unwrap()
								.set(smr, false);
						} else {
	...
	fn alloc_ctxbnk(&mut self) -> Option<usize> {
		let bitmap = match &mut self.context_alloc_bitmap {
			None => panic!(
				"smmu_alloc_ctxbnk: smmu v2 context_alloc_bitmap not init"
			),
			Some(bitmap) => bitmap,
		};
	...
	fn write_ctxbnk(
		...
		if self.context_alloc_bitmap.is_none()
			|| self.context_alloc_bitmap.as_ref().unwrap().get(context_id) == 0
		{
		...
		let tcr =
			if cfg!(feature = "lvl4") {
				((ps & 0x7) << SMMUV2_TCR_PS_OFF)
					| (t0sz & 0x1F) | SMMUV2_TCR_TG0_4K
					| SMMUV2_TCR_ORGN0_WB_RA_WA
					| SMMUV2_TCR_IRGN0_WB_RA_WA
					| SMMUV2_TCR_SH0_IS | SMMUV2_TCR_SL0_01
			} else {
				((ps & 0x7) << SMMUV2_TCR_PS_OFF)
					| (t0sz & 0x1F) | SMMUV2_TCR_TG0_4K
					| SMMUV2_TCR_ORGN0_WB_RA_WA
					| SMMUV2_TCR_IRGN0_WB_RA_WA
					| SMMUV2_TCR_SH0_IS | SMMUV2_TCR_SL0_12
			};
...
pub fn smmu_vm_init(vm: &Vm) -> bool {
	...
	match smmu_v2.alloc_ctxbnk() {
		...
		None => {
			panic!("iommu: smmuv2 could not allocate ctx for vm[{}]", vm.id())
		}
	}
}
...
pub fn smmu_add_device(context_id: usize, stream_id: usize) -> bool {
	let mut smmu_v2 = SMMU_V2.get().unwrap().lock();
	let prep_id =
		(stream_id & bit_mask!(SMMU_SMR_ID_OFF, SMMU_SMR_ID_LEN)) as u16;
	...
	if !emu_ctx.write {
		...
		if permit_write {
			...
			info!(
				...
				active_vm_id(),
				(address - smmu_v2.context_bank.first().unwrap().addr())
					/ 0x10000,
			);
		}
	}
