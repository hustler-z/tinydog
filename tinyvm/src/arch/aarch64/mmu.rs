...
impl BlockDescriptor {
	fn new(output_addr: usize, mem_type: MemoryType) -> BlockDescriptor {
		BlockDescriptor(
			(PageDescriptorS1::OUTPUT_PPN
				.val((output_addr >> PAGE_SHIFT) as u64)
				...
				+ match mem_type {
					MemoryType::Device => {
						PageDescriptorS1::AttrIndx::Attr0
							+ PageDescriptorS1::SH::OuterShareable
					}
					MemoryType::Normal => {
						PageDescriptorS1::AttrIndx::Attr1
							+ PageDescriptorS1::SH::InnerShareable
					}
				})
			.value,
		)
...
pub extern "C" fn pt_populate(
	lvl1_pt: &mut PageTables,
	lvl2_pt: &mut PageTables,
) {
	...
	#[cfg(feature = "qemu")]
	{
		...
		for index in 0..PLATFORM_PHYSICAL_LIMIT_GB {
			...
			if pa < PLAT_DESC.mem_desc.base {
				lvl1_pt.entry[index] =
					BlockDescriptor::new(pa, MemoryType::Device);
			} else {
				lvl1_pt.entry[index] =
					BlockDescriptor::new(pa, MemoryType::Normal);
			}
		}
	...
	#[cfg(feature = "rk3588")]
	{
		...
		const DEVICE_BOUND: usize =
			(0xf000_0000 - 0xc000_0000) / (1 << LVL2_SHIFT);
		for i in 0..DEVICE_BOUND {
			lvl2_pt.entry[i] = BlockDescriptor::new(
				0x0c0000000 + (i << LVL2_SHIFT),
				MemoryType::Normal,
			);
		}
		...
		lvl1_pt.entry[4] =
			BlockDescriptor::new(0x100000000, MemoryType::Normal);
		lvl1_pt.entry[5] =
			BlockDescriptor::new(0x140000000, MemoryType::Normal);
		lvl1_pt.entry[6] =
			BlockDescriptor::new(0x180000000, MemoryType::Normal);
		lvl1_pt.entry[7] =
			BlockDescriptor::new(0x1c0000000, MemoryType::Normal);
		for i in 8..512 {
