...
use crate::arch::{gic_cpu_reset, gicc_clear_current_irq, GIC_PRIVINT_NUM};
#[cfg(not(feature = "gicv3"))]
use crate::board::{PlatOperation, Platform};
use crate::kernel::{current_cpu, Vcpu, Vm};

use super::{gicc_get_current_irq, GICD, GIC_SGIS_NUM};
...
impl InterruptController for IntCtrl {
	...
	fn init() {
		use crate::arch::{
			gic_cpu_init, gic_glb_init, gic_maintenance_handler,
		};
	...
	fn enable(int_id: usize, en: bool) {
		#[cfg(feature = "gicv3")]
		{
			...
			GICD.set_route(
				int_id,
				cortex_a::registers::MPIDR_EL1.get() as usize,
			);
		}
		#[cfg(not(feature = "gicv3"))]
		{
			if en {
				...
				GICD.set_trgt(
					int_id,
					1 << Platform::cpuid_to_cpuif(current_cpu().id),
				);
