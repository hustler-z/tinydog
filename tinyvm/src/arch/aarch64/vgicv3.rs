...
use crate::device::{emu_register_reg, EmuDev, EmuDeviceType, EmuRegType};
use crate::kernel::{active_vm, active_vm_id};
use crate::kernel::{
	cpuid2mpidr, current_cpu, restore_vcpu_gic, save_vcpu_gic, IpiInitcMessage,
};
use crate::kernel::{
	ipi_intra_broadcast_msg, ipi_send_msg, IpiInnerMsg, IpiMessage, IpiType,
};
...
impl VgicInt {
	...
	fn priv_new(
		...
		Self {
			...
			inner: Mutex::new(VgicIntInnerMut::priv_new(
				owner, targets, enabled, redist, cfg,
			)),
			lock: Mutex::new(()),
		}
	}
...
impl Vgicd {
	fn new(cpu_num: usize) -> Vgicd {
		Vgicd {
			...
			typer: (GICD.typer()
				& !(GICD_TYPER_CPUNUM_MSK | GICD_TYPER_LPIS) as u32)
				| ((((cpu_num - 1) << GICD_TYPER_CPUNUM_OFF)
					& GICD_TYPER_CPUNUM_MSK) as u32),
			iidr: GICD.iidr(),
			interrupts: Vec::new(),
		}
	}
...
impl VgicCpuPrivMut {
	fn queue_remove(
		list: &mut VecDeque<NonNull<VgicInt>>,
		interrupt: &VgicInt,
	) {
	...
	fn pend_list_push(&mut self, interrupt: &VgicInt) {
		/// SAFETY: All VgicInt are allocated when initializing, so it's safe to convert them to NonNull
		self.pend_list.push_back(unsafe {
			NonNull::new_unchecked(interrupt as *const _ as *mut _)
		});
	}
	...
	fn act_list_push(&mut self, interrupt: &VgicInt) {
		/// SAFETY: All VgicInt are allocated when initializing, so it's safe to convert them to NonNull
		self.act_list.push_back(unsafe {
			NonNull::new_unchecked(interrupt as *const _ as *mut _)
		});
	}
...
impl Vgic {
	...
	fn vgicr_emul_typer_access(&self, emu_ctx: &EmuContext, vgicr_id: usize) {
		if !emu_ctx.write {
			...
			current_cpu().set_gpr(
				emu_ctx.reg,
				self.cpu_priv[vgicr_id].vigcr.get_typer() as usize,
			);
		}
	}
			...
			if vgic_int_get_owner(vcpu, interrupt) {
				...
				let phys_route = if (val & GICD_IROUTER_IRM_BIT) != 0 {
					...
					match vcpu
						.vm()
						.unwrap()
						.get_vcpu_by_mpidr(val & MPIDR_AFF_MSK)
					{
						Some(vcpu) => {
							cpuid2mpidr(vcpu.phys_id()) & MPIDR_AFF_MSK
						}
						_ => GICD_IROUTER_INV,
					}
				};
	...
	fn remove_lr(&self, vcpu: &Vcpu, interrupt: &VgicInt) -> bool {
		...
		let lr_state =
			bit_extract(lr_val, GICH_LR_STATE_OFF, GICH_LR_STATE_LEN);
		if lr_state != IrqState::IrqSInactive.to_num() {
			...
			if (interrupt.state().to_num() & IrqState::IrqSPend.to_num() != 0)
				&& interrupt.enabled()
			{
				let hcr = GICH.hcr();
	...
	fn add_lr(&self, vcpu: &Vcpu, interrupt: &VgicInt) -> bool {
		...
		match lr_ind {
			...
			None => {
				...
				{
					let hcr = GICH.hcr();
					// No Pending Interrupt Enable. Enables the signaling of a
					// maintenance interrupt when there are no List registers
					// with the State field set to 0b01 then a maintance
					// interrupt will come.
					GICH.set_hcr(hcr | GICH_HCR_NPIE_BIT);
				}
			}
	...
	fn route(&self, vcpu: &Vcpu, interrupt: &VgicInt) {
		...
		if !interrupt.in_lr() && vgic_int_has_other_target(interrupt) {
			...
			vgic_int_yield_owner(vcpu, interrupt);
			let trglist =
				vgic_int_ptarget_mask(interrupt) & !(1 << vcpu.phys_id());
		...
		if let Some(interrupt) = self.get_int(vcpu, bit_extract(int_id, 0, 10))
		{
			...
			if vgic_int_get_owner(vcpu, interrupt) {
				...
				if act && ((state & IrqState::IrqSActive.to_num()) == 0) {
					interrupt.set_state(IrqState::num_to_state(
						state | IrqState::IrqSActive.to_num(),
					));
	...
	fn emu_ctrl_access(&self, emu_ctx: &EmuContext) {
		if emu_ctx.write {
			...
			let idx = emu_ctx.reg;
			self.set_vgicd_ctlr(
				current_cpu().get_gpr(idx) as u32 & 0x2
					| GICD_CTLR_ARE_NS_BIT as u32,
			);
	...
	fn emu_typer_access(&self, emu_ctx: &EmuContext) {
		if !emu_ctx.write {
			...
			error!("can't write to read only reg");
		}
	}
	...
	fn emu_isenabler_access(&self, emu_ctx: &EmuContext) {
		...
		for i in 0..(emu_ctx.width * 8) {
			if vm.has_interrupt(first_int + i)
				|| vm.emu_has_interrupt(first_int + i)
			{
		...
		if emu_ctx.write {
			...
			for i in 0..32 {
				match self.get_int(
					...
					Some(interrupt) => {
						if vgic_get_state(interrupt)
							& IrqState::IrqSPend.to_num()
	...
	fn emu_pidr_access(&self, emu_ctx: &EmuContext) {
		if !emu_ctx.write {
			current_cpu().set_gpr(
				emu_ctx.reg,
				GICD.id(((emu_ctx.address & 0xff) - 0xd0) / 4) as usize,
			);
		}
	}
	...
	fn emu_probaser_access(&self, emu_ctx: &EmuContext) {
		if emu_ctx.write {
			...
			current_cpu().set_gpr(
				emu_ctx.reg,
				GICR.get_propbaser(current_cpu().id) as usize,
			);
		}
	}
	...
	fn emu_pendbaser_access(&self, emu_ctx: &EmuContext) {
		if emu_ctx.write {
			...
			current_cpu().set_gpr(
				emu_ctx.reg,
				GICR.get_pendbaser(current_cpu().id) as usize,
			);
		}
	}
	...
	fn emu_icfgr_access(&self, emu_ctx: &EmuContext) {
		let first_int =
			((emu_ctx.address & 0xffff) - 0x0C00) * 8 / GIC_CONFIG_BITS; // emu_ctx.address - OFFSET(GICR/D,ICFGR)
		...
		if emu_ctx.write {
			...
			while bit < (emu_ctx.width * 8) {
				cfg |= (self
					.get_icfgr(current_cpu().active_vcpu.as_ref().unwrap(), irq)
					as usize) << bit;
				bit += 2;
				irq += 1;
			}
	...
	fn emu_ipriorityr_access(&self, emu_ctx: &EmuContext) {
		let first_int =
			((emu_ctx.address & 0xffff) - 0x0400) * 8 / GIC_PRIO_BITS; // emu_ctx.address - OFFSET(GICR/D,IPRIORITYR)
		let idx = emu_ctx.reg;
		let mut val = if emu_ctx.write {
			current_cpu().get_gpr(idx)
		} else {
	...
	fn handle_trapped_eoir(&self, vcpu: &Vcpu) {
		...
		let mut lr_idx_opt =
			bitmap_find_nth(GICH.eisr() as usize, 0, gic_lrs, 1, true);
		...
		while lr_idx_opt.is_some() {
			...
			match self.get_int(
				vcpu,
				bit_extract(lr_val, GICH_LR_VID_OFF, GICH_LR_VID_LEN),
			) {
				Some(interrupt) => {
					let interrupt_lock = interrupt.lock.lock();
					interrupt.set_in_lr(false);
	...
	fn vgic_highest_proi_spilled(
		&self,
		vcpu: &Vcpu,
		flag: bool,
	) -> Option<NonNull<VgicInt>> {
		...
		binding
			.min_by_key(|x| {
				((unsafe { x.as_ref().prio() } as u32) << 10)
					| unsafe { x.as_ref().id() } as u32
			})
			.cloned()
	}
	...
	fn refill_lrs(&self, vcpu: &Vcpu, flag: bool) {
		let gic_lrs = gic_lrs();
		// ICH_ELRSR_EL2:locate a usable List register when the hypervisor is
		// delivering an interrupt to a Guest OS.
		let mut lr_idx_opt = bitmap_find_nth(GICH.elrsr(), 0, gic_lrs, 1, true);
		// flag indicates that is no pending or not true:no pending flase:have pending,
		// the we will look up active and pend
		let mut new_flags = flag;
		while lr_idx_opt.is_some() {
			match self.vgic_highest_proi_spilled(vcpu, new_flags) {
				Some(interrupt_ptr) => {
					/// SAFETY: All VgicInt are allocated when initializing, so it's
					/// safe to convert them to NonNull
	...
	fn eoir_highest_spilled_active(&self, vcpu: &Vcpu) {
		...
		if let Some(int_ptr) = interrupt {
			/// SAFETY: All VgicInt are allocated when initializing, so
			/// it's safe to convert them to NonNull
...
fn vgicr_get_id(emu_ctx: &EmuContext) -> u32 {
	((emu_ctx.address - PLAT_DESC.arch_desc.gic_desc.gicr_addr)
		/ size_of::<GicRedistributor>()) as u32
}
...
fn vgicr_emul_ctrl_access(emu_ctx: &EmuContext) {
	if !emu_ctx.write {
		current_cpu().set_gpr(
			emu_ctx.reg,
			GICR.get_ctrl(current_cpu().id as u32) as usize,
		);
	} else {
...
fn vgic_int_vcpu_is_target(vcpu: &Vcpu, interrupt: &VgicInt) -> bool {
	...
	let routed_here = !pri
		&& (interrupt.phys_route() as usize
			^ (MPIDR_EL1::read() as usize & MPIDR_AFF_MSK))
			== 0;
...
fn vgic_get_state(interrupt: &VgicInt) -> usize {
	...
	if interrupt.in_lr()
		&& interrupt.owner_phys_id().unwrap() == current_cpu().id
	{
...
fn gich_get_lr(interrupt: &VgicInt) -> Option<usize> {
	...
	if (bit_extract(lr_val, GICH_LR_VID_OFF, GICH_LR_VID_LEN)
		== interrupt.id() as usize)
		&& (bit_extract(lr_val, GICH_LR_STATE_OFF, GICH_LR_STATE_LEN)
			!= IrqState::IrqSInactive.to_num())
	{
...
pub fn gic_maintenance_handler() {
	...
	// U: underflow Zero or one of the List register entries are marked as a valid interrupt,
	// that is, if the corresponding ICH_LR<n>_EL2.State bits do not equal 0x0.
	...
	if misr & (GICH_MISR_LRPEN as u32) != 0 {
		...
		while hcr & GICH_HCR_EOIC_MSK != 0 {
			vgic.eoir_highest_spilled_active(
				current_cpu().active_vcpu.as_ref().unwrap(),
			);
...
pub fn partial_passthrough_intc_init(
	...
	if let IpiInnerMsg::Initc(intc) = msg.ipi_message {
		...
		let trgt_vcpu =
			match current_cpu().vcpu_array.pop_vcpu_through_vmid(vm_id) {
				None => {
					error!(
						"hcpu[{}] received vgic msg from unknown vm[{}]",
						current_cpu().id,
						vm_id
					);
					return;
		...
		match intc.event {
			...
			InitcEvent::VgicdRoute => {
				if let Some(interrupt) =
					vgic.get_int(trgt_vcpu, bit_extract(int_id as usize, 0, 10))
				{
					let interrupt_lock = interrupt.lock.lock();
			...
			InitcEvent::Vgicdinject => {
				crate::kernel::interrupt_vm_inject(
					trgt_vcpu.vm().as_ref().unwrap(),
					trgt_vcpu,
					int_id as usize,
				);
			}
			_ => {
				error!("hcpu[{}] received unknown event", current_cpu().id)
			}
		}
...
pub fn emu_intc_init(
	...
	for vcpu in vcpu_list {
		...
		typer |=
			!!((vcpu.id() == (vcpu_num - 1)) as usize) << GICR_TYPER_LAST_OFF;

		// need the low 6 bits for LPI/ITS init
		// DPGS, bit [5]:Sets support for GICR_CTLR.DPG* bits
		// DirectLPI, bit [3]: Indicates whether this Redistributor supports direct
		// injection of LPIs.
		// Dirty, bit [2]: Controls the functionality of GICR_VPENDBASER.Dirty.
		// LPI VLPIS, bit [1]: Indicates whether the GIC implementation supports
		// virtual LPIs and the direct injection of virtual LPIs
		// PLPIS, bit [0]: Indicates whether the GIC implementation supports
		// physical LPIs
...
impl EmuDev for VgicRedis {
	...
	fn handler(&self, emu_ctx: &EmuContext) -> bool {
		...
		trace!(
            "hcpu[{}] => addr:{:#x} reg {:?} offset {:#x} is write:{},val:{:#x}",
            current_cpu().id,
            emu_ctx.address,
...
pub fn emu_vgicr_init(
	emu_cfg: &VmEmulatedDeviceConfig,
	vgic: Arc<Vgic>,
) -> Result<Arc<dyn EmuDev>, ()> {
	if emu_cfg.emu_type != EmuDeviceType::EmuDeviceTGICR {
		error!("emulate type is not EmuDeviceTGICR");
		return Err(());
	}

	let vgicr =
		Arc::new(VgicRedis::new(emu_cfg.base_ipa, emu_cfg.length, vgic));
...
pub fn vgic_icc_sgir_handler(_emu_dev_id: usize, emu_ctx: &EmuContext) -> bool {
	if emu_ctx.write {
		...
		let int_id =
			bit_extract(sgir, GICC_SGIR_SGIINTID_OFF, GICC_SGIR_SGIINTID_LEN);
