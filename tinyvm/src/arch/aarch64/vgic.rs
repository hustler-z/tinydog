...
use crate::kernel::{
	active_vcpu_id, current_cpu, restore_vcpu_gic, save_vcpu_gic,
};
...
impl Vgic {
		...
		if lr_state != 0 {
			...
			if int_id < GIC_SGIS_NUM {
				if interrupt.state().to_num() & 2 != 0 {
					self.set_cpu_priv_sgis_act(
						vcpu_id,
						int_id,
						((lr_val >> 10) & 0b111) as u8,
					);
		...
		if lr_ind.is_none() {
			...
			if let Some(idx) = lr_ind {
				let spilled_int = self
					.get_int(vcpu, GICH.lr(idx) as usize & 0b1111111111)
					.unwrap();
	...
	fn write_lr(&self, vcpu: &Vcpu, interrupt: &VgicInt, lr_ind: usize) {
		...
		let mut lr = (int_id & 0b1111111111)
			| (((int_prio as usize >> 3) & 0b11111) << 23);
		...
		if vgic_int_is_hw(interrupt) {
			...
			if (state & 2) != 0 {
				lr |= ((self.cpu_priv_sgis_act(vcpu_id, int_id) as usize)
					<< 10) & (0b111 << 10);
				lr |= (2 & 0b11) << 28;
			} else {
				let mut idx = GIC_TARGETS_MAX - 1;
				while idx as isize >= 0 {
					if (self.cpu_priv_sgis_pend(vcpu_id, int_id) & (1 << idx))
						!= 0
					{
						lr |= (idx & 0b111) << 10;
						let pend = self.cpu_priv_sgis_pend(vcpu_id, int_id);
				...
				if interrupt.targets() != trgt {
					...
					for cpuid in 0..8 {
						if bit_get(trgt as usize, cpuid) != 0 {
							ptrgt =
								bit_set(ptrgt, Platform::cpuid_to_cpuif(cpuid))
						}
					}
					if interrupt.hw() {
	...
	fn emu_icfgr_access(&self, emu_ctx: &EmuContext) {
		let first_int =
			(32 / GIC_CONFIG_BITS) * bit_extract(emu_ctx.address, 0, 9) / 4;
		let vm_id = active_vm_id();
		let vm = match active_vm() {
			Some(vm) => vm,
			None => {
	...
	fn emu_sgiregs_access(&self, emu_ctx: &EmuContext) {
		...
		if bit_extract(emu_ctx.address, 0, 12)
			== bit_extract(Platform::GICD_BASE + 0x0f00, 0, 12)
		{
			if emu_ctx.write {
				...
				for i in 0..8 {
					if trgtlist & (1 << i) != 0 {
						let m = IpiInitcMessage {
							event: InitcEvent::VgicdSetPend,
							vm_id: active_vm_id(),
							int_id: (bit_extract(val, 0, 8)
								| (active_vcpu_id() << 10)) as u16,
							val: true as u8,
						};
						if !ipi_send_msg(
							i,
	...
	fn emu_ipriorityr_access(&self, emu_ctx: &EmuContext) {
		...
		let first_int =
			(8 / GIC_PRIO_BITS) * bit_extract(emu_ctx.address, 0, 9);
		let vm_id = active_vm_id();
		let vm = match active_vm() {
			Some(vm) => vm,
			None => {
	...
	fn emu_itargetr_access(&self, emu_ctx: &EmuContext) {
		...
		let first_int =
			(8 / GIC_TARGET_BITS) * bit_extract(emu_ctx.address, 0, 9);

		if emu_ctx.write {
			val = vgic_target_translate(&active_vm().unwrap(), val as u32, true)
				as usize;
			for i in 0..emu_ctx.width {
				self.set_trgt(
					current_cpu().active_vcpu.as_ref().unwrap(),
			...
			for i in 0..emu_ctx.width {
				val |= (self.get_trgt(
					current_cpu().active_vcpu.as_ref().unwrap(),
					first_int + i,
				) as usize) << (GIC_TARGET_BITS * i);
			}
			val =
				vgic_target_translate(&active_vm().unwrap(), val as u32, false)
					as usize;
			let idx = emu_ctx.reg;
			current_cpu().set_gpr(idx, val);
		}
