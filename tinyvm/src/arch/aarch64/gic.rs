...
impl GicDistributor {
	...
	pub fn set_icfgr(&self, int_id: usize, cfg: u8) {
		...
		self.ICFGR[reg_ind]
			.set((icfgr & !mask) | (((cfg as u32) << off) & mask));
		drop(lock);
	}
...
pub static GICH: DeviceRef<GicHypervisorInterface> = unsafe {
	DeviceRef::new(Platform::GICH_BASE as *const GicHypervisorInterface)
};
