...
use crate::device::{
	virtio_blk_notify_handler, virtio_console_notify_handler,
	virtio_mediated_blk_notify_handler, virtio_net_handle_ctrl,
	virtio_net_notify_handler, EmuContext, EmuDev, EmuDeviceType,
};
use crate::device::{
	VIRTQUEUE_BLK_MAX_SIZE, VIRTQUEUE_CONSOLE_MAX_SIZE, VIRTQUEUE_NET_MAX_SIZE,
};
use crate::kernel::Vm;
use crate::kernel::{active_vm, active_vm_id};
use crate::kernel::{
	current_cpu, ipi_send_msg, vm_ipa2pa, IpiInnerMsg, IpiIntInjectMsg, IpiType,
};
...
impl VirtioMmio {
	...
	fn virtio_queue_init(
		&mut self,
		weak: &Weak<VirtioMmio>,
		dev_type: VirtioDeviceType,
	) {
		match dev_type {
			...
			VirtioDeviceType::Net => {
				...
				for i in 0..2 {
					let queue =
						Virtq::new(i, weak.clone(), virtio_net_notify_handler);
					self.inner_const.vq.push(queue);
				}
...
fn virtio_mmio_prologue_access(
	mmio: &VirtioMmio,
	emu_ctx: &EmuContext,
	offset: usize,
	write: bool,
		...
		match offset {
			...
			VIRTIO_MMIO_QUEUE_DESC_HIGH => match mmio.vq(q_sel) {
				Ok(virtq) => {
					...
					let desc_table_addr = vm_ipa2pa(
						&active_vm().unwrap(),
						virtq.desc_table_addr(),
					);
			...
			VIRTIO_MMIO_QUEUE_AVAIL_HIGH => match mmio.vq(q_sel) {
				Ok(virtq) => {
					...
					let avail_addr =
						vm_ipa2pa(&active_vm().unwrap(), virtq.avail_addr());
					if avail_addr == 0 {
			...
			VIRTIO_MMIO_QUEUE_USED_HIGH => match mmio.vq(q_sel) {
				Ok(virtq) => {
					...
					let used_addr =
						vm_ipa2pa(&active_vm().unwrap(), virtq.used_addr());
					if used_addr == 0 {
			...
			_ => {
				error!(
					"virtio mmio queue: wrong reg write 0x{:x}",
					emu_ctx.address
				);
			}
		}
		...
		let value = match offset {
			...
			VIRTIO_MMIO_CONFIG..=0x1ff => match mmio.dev().desc() {
				super::DevDesc::BlkDesc(blk_desc) => {
					...
					unsafe {
						blk_desc.offset_data(offset - VIRTIO_MMIO_CONFIG, width)
					}
				}
				super::DevDesc::NetDesc(net_desc) => {
					// SAFETY: Offset is between VIRTIO_MMIO_CONFIG..VIRTIO_MMIO_REGS_END ,so is valid
					unsafe {
						net_desc.offset_data(offset - VIRTIO_MMIO_CONFIG, width)
					}
				}
				_ => {
...
impl EmuDev for VirtioMmio {
	...
	fn handler(&self, emu_ctx: &EmuContext) -> bool {
		...
		} else if (VIRTIO_MMIO_MAGIC_VALUE..=VIRTIO_MMIO_GUEST_FEATURES_SEL)
			.contains(&offset)
			|| offset == VIRTIO_MMIO_STATUS
		{
			virtio_mmio_prologue_access(self, emu_ctx, offset, write);
		} else if (VIRTIO_MMIO_QUEUE_SEL..=VIRTIO_MMIO_QUEUE_USED_HIGH)
			.contains(&offset)
		{
			virtio_mmio_queue_access(self, emu_ctx, offset, write);
		} else if (VIRTIO_MMIO_CONFIG_GENERATION..=VIRTIO_MMIO_REGS_END)
			.contains(&offset)
		{
