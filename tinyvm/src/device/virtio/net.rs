...
use crate::device::{
	DevDesc, VirtioMmio, Virtq, VIRTQ_DESC_F_NEXT, VIRTQ_DESC_F_WRITE,
};
...
pub fn virtio_net_handle_ctrl(
	...
	while let Some(next_desc_idx) = vq.pop_avail_desc_idx(vq.avail_idx()) {
		...
		out_iov
			.to_buf(&ctrl as *const _ as usize, size_of::<VirtioNetCtrlHdr>());
		match ctrl.class {
...
pub fn virtio_net_notify_handler(
	vq: Arc<Virtq>,
	nic: Arc<VirtioMmio>,
	vm: Arc<Vm>,
) -> bool {
	...
	for nic in nics_to_notify {
		...
		if vcpu.phys_id() == current_cpu().id {
			let rx_vq = match nic.vq(0) {
				...
				Err(_) => {
					error!(
						"vm[{}] failed to get virtio net rx virt queue",
						vm.id()
					);
					return false;
				}
...
fn ethernet_transmit(
	tx_iov: VirtioIov,
	len: usize,
	vm: &Vm,
) -> Option<Vec<Arc<VirtioMmio>>> {
	// [ destination MAC - 6 ][ source MAC - 6 ][ EtherType - 2 ][ Payload ]
	if len < size_of::<VirtioNetHdr>()
		|| len - size_of::<VirtioNetHdr>() < 6 + 6 + 2
	{
		warn!(
