...
use crate::kernel::{
	active_vm_id, add_async_task, async_blk_id_req, async_blk_io_req,
	async_ipi_req, push_used_info, AsyncTask, AsyncTaskData, AsyncTaskState,
	IoAsyncMsg, IoIdAsyncMsg, IpiMediatedMsg, Vm,
};
...
pub fn generate_blk_req(
	...
	for req_node in req_node_list {
		...
		if sector + req_node.iov_sum_up / SECTOR_BSIZE
...
pub fn virtio_blk_notify_handler(
	vq: Arc<Virtq>,
	blk: Arc<VirtioMmio>,
	vm: Arc<Vm>,
) -> bool {
		...
		loop {
			if vq.desc_has_next(next_desc_idx) {
				if head {
					...
					let vreq =
						unsafe { &mut *(vreq_addr as *mut VirtioBlkReqNode) };
					req_node.req_type = vreq.req_type;
					req_node.sector = vreq.sector;
				} else {
					/*data handler*/
					if (vq.desc_flags(next_desc_idx) & 0x2) as u32 >> 1
						== req_node.req_type
					{
						error!(
				...
				if req_node.req_type > 1
					&& req_node.req_type != VIRTIO_BLK_T_GET_ID as u32
				{
