...
use super::{
	AddrRegions, DtbDevType, PassthroughRegion, VMDtbDevConfigList,
	VmConfigEntry, VmCpuConfig, VmDtbDevConfig, VmEmulatedDeviceConfig,
...
pub fn init_tmp_config_for_bma1() {
	...
	// bma passthrough
	let mut pt_dev_config: VmPassthroughDeviceConfig =
		VmPassthroughDeviceConfig::default();
...
pub fn init_tmp_config_for_vm1() {
	...
	let vm1_config = VmConfigEntry {
		...
		cmdline: String::from(
			"earlycon console=hvc0,115200n8 root=/dev/vda rw audit=0",
		),
...
pub fn init_tmp_config_for_vm2() {
	...
	let vm2_config = VmConfigEntry {
		...
		// cmdline: "root=/dev/vda rw audit=0",
		cmdline: String::from(
			"earlycon console=ttyS0,115200n8 root=/dev/vda rw audit=0",
		),
