...
use crate::kernel::{active_vm, vm, vm_ipa2pa, Vm, VmType, VM_NUM_MAX};
use crate::utils::{memcpy, BitAlloc, BitAlloc16};
...
pub static DEF_VM_CONFIG_TABLE: Mutex<VmConfigTable> =
	Mutex::new(VmConfigTable::new());
...
pub fn vm_cfg_add_vm_entry(
	mut vm_cfg_entry: VmConfigEntry,
) -> Result<usize, ()> {
	let mut vm_config = DEF_VM_CONFIG_TABLE.lock();
	match vm_config.generate_vm_id() {
		Ok(vm_id) => {
			if vm_id == 0
				&& (!vm_config.entries.is_empty() || vm_config.vm_num > 0)
			{
...
pub fn vm_cfg_add_vm(config_ipa: usize) -> Result<usize, ()> {
	...
	// SAFETY: config_pa is from user space, it is checked by tinyvm.ko firstly.
...
pub fn vm_cfg_add_passthrough_device_streams_ids(
	...
	let streams_ids_base_pa =
		vm_ipa2pa(&active_vm().unwrap(), streams_ids_base_ipa);
...
fn vm_cfg_finish_configuration(
	vmid: usize,
	_img_size: usize,
) -> alloc::sync::Arc<Vm> {
...
pub fn vm_cfg_upload_kernel_image(
	...
	let src = unsafe {
		core::slice::from_raw_parts_mut(cache_pa as *mut u8, load_size)
	};

	let dst = unsafe {
		core::slice::from_raw_parts_mut(dest_pa as *mut u8, load_size)
	};
...
pub fn vm_cfg_upload_device_tree(
	...
	// SAFETY: We have checked the cache_pa is in the memory region of the VM by vm_ipa2pa.
	let src =
		unsafe { core::slice::from_raw_parts(cache_pa as *mut u8, load_size) };
