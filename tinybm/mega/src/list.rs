...
/// Interior Mutability
///
/// Allow to mutate a value through a shared reference
///
/// [1] Mutex / RefCell - Offer safety mechanisms to ensure that, for any value
///                       they give a mutable reference to, only one mutable
///                       reference (and no shared references) can exist at a
///                       time.
/// [2] atomic / Cell   - Offer methods for manipulating the value in place.
///
///
///
/// RefCell, Cell, etc, internally use 'UnsafeCell' to wrap their data.
///
...
///              &UnsafCell<T> May point to data that is being mutated.
///
///
///
/// ############################################################################
...
/// further progress by polling again. The context passed to the poll method
/// can provide a Waker, which is a handle for waking up the current task.
...
///          +---------+
///
/// [N] Poll if woken task make progress
///
/// ############################################################################
...
/// @Hustler 2024/11/19
///
/// ManuallyDrop - A wrapper to inhibit the compiler from automatically calling
///                T's destructor.
///
///
///
...
/// pin
///
/// Types that pin data to a location in memory. It guarantee that the value a
/// pointer is pointing at will [1] not be moved out of its memory location,
/// [2] more generally, remain valid at that same memory location.
///
/// Pin is Specifically targeted at allowing the implementation of safe
/// interfaces around types which have some state during which they become
/// "address-sensitive". A value in such an “address-sensitive” state is not
/// okay with being moved around at-will. Such a value must stay un-moved
/// and valid during the address-sensitive portion of its lifespan because
/// some interface is relying on those invariants to be true in order for
/// its implementation to be sound.
///
///  +---------+
///  | Address | (pinned)
///  +---------+
...
/// @Hustler 2024/11/16
///
/// Waker is a handle for waking up a task by notifying its executor that it is
/// ready to be run.
///
/// RawWaker defines the executor-specific wakeup behavior.
///
///             +-------------------+
///             | Context           |
///             | +--------------+  |
...
///       +---------------------------------+
///
/// (Clone, Send, Sync implemented for Waker)
///
/// ############################################################################
...
use crate::util::{Captures, NotSendMarker};
...
impl<T> Node<T> {
	...
	pub unsafe fn new(contents: T, waker: Waker) -> ManuallyDrop<Self> {
		...
		unsafe { Self::new_with_meta(contents, (), waker) }
	}
}
...
fn debug_link<'a, T>(
	parent: &T,
	cell: &'a Cell<NonNull<T>>,
) -> &'a dyn core::fmt::Debug {
	#[derive(Debug)]
...
impl<T: PartialOrd, M> List<T, M> {
	...
	pub fn wake_while(
		self: Pin<&Self>,
		mut pred: impl FnMut(Pin<&Node<T, M>>) -> bool,
	) -> bool {
	...
	pub fn wake_one_if(
		self: Pin<&Self>,
		pred: impl FnOnce(Pin<&Node<T, M>>) -> bool,
	) -> bool {
...
impl<T: PartialOrd, M, F: FnOnce()> Future
	for WaitWithCleanup<'_, '_, T, M, F>
{
	type Output = ();
...
impl<T, M, F: FnOnce()> PinnedDrop for WaitWithCleanup<'_, '_, T, M, F> {
	fn drop(self: Pin<&mut Self>) {
		if self.inner.state.get() == WaitState::Attached
			&& self.inner.node.is_detached()
		{
...
macro_rules! create_list_with_meta {
	($var:ident, $met:expr) => {
		...
		let mut $var = core::pin::pin!(unsafe {
			core::mem::ManuallyDrop::into_inner(
				$crate::list::List::new_with_meta($var),
			)
		});
	...
	($var:ident) => {
		$crate::create_list_with_meta!($var, core::default::Default::default())
	};
}
...
macro_rules! create_node_with_meta {
	($var:ident, $dl:expr, $meta:expr, $w: expr) => {
		...
		let mut $var = core::pin::pin!(unsafe {
			core::mem::ManuallyDrop::into_inner(
				$crate::list::Node::new_with_meta($var.0, $var.1, $var.2),
			)
		});
		// Safety: the value has not been operated on since `new_with_meta`
		// except for being pinned, so this operation causes it to become valid
